...
The forwarding plane also contains the \textit{forwarding fabric} which is the complete collection of all possible paths packets can take between interfaces.  At runtime the specific path to be taken is determined by consulting the routing table.
...
These layers were all implemented within the receiving thread, with each layer being a function, and the Ethernet function sending the packet to the correct transmitting thread (as described in the last paragraph).
