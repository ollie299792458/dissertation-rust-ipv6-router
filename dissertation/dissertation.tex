...
Luyou was implemented by first creating the framework, and then adding features requirement by requirement (as set out at the end of \hyperref[sec::analysis]{Analysis }\ref{sec::analysis}).  Version control was performed using git\cite{git}, with backups committed to an \textit{exploratory} branch and merges into the \textit{master} branch whenever a specific requirement had been completely implemented and tested. 
...
\item Forwarding Fabric: \verb!HashMap<MacAddr,Sender<EthernetPacket>>!
\end{itemize}

Finally I needed a representation for the different types of packets I would be dealing with, pnet includes structures like \verb!MutableEthernetPacket! \& \verb!IPv6Packet! that wrap around a slice of the byte buffer received from the network interface and allow the packet fields to be easily accessed.

Routing struct

\bigskip

Then I implemented the channels

DataLinkSender

annoyance with type - need transmitting threads

annoyance with packet cloning

\bigskip

Then I implemented the functions

pnet packet allocation annoyance

Then I added the features

\begin{figure}
\begin{lstlisting}[style=tree]
Main Thread
├── Control Thread
└── Forwarding Threads - a pair per interface
    ├── Receiving Thread
    └── Transmitting Thread
