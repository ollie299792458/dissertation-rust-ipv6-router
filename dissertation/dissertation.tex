...
Luyou was implemented by first creating the framework, and then adding features requirement by requirement (as set out at the end of \hyperref[sec::analysis]{Analysis }\ref{sec::analysis}).  Version control was performed using git\cite{git}, with backups committed to an \textit{exploratory} branch and merges into the \textit{master} branch whenever a specific requirement had been completely implemented and tested.  See \hyperref[fig::implementation_overview]{Figure }\ref{fig::implementation_overview} for an overview of data structures and threads in the implementation.
...
After implementing the initial skeleton of the above framework I began implementing the requirements one by one.  The first was writing an Ethernet repeater, this demonstrated that pnet\cite{pnet} and my framework worked as expected. I then implemented each requirement in turn, modifying the skeleton if necessary. 

\bigskip

The static routing rules were read from a text file that was created by Luxing. See \hyperref[fig::config_file]{Figure }\ref{fig::config_file} for details of the file format.
